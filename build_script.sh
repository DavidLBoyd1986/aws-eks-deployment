...
aws cloudformation deploy --stack-name eks-hacking-bh-vpc-stack \
    --template-file ./IaC_Templates/bastion_host_vpc_deployment.yml
...
aws cloudformation deploy --stack-name eks-hacking-eks-vpc-stack \
    --template-file ./IaC_Templates/eks_vpc_deployment.yml

echo "Deploy the EKS IAM stack"
aws cloudformation deploy --stack-name eks-hacking-eks-iam-stack --template-file ./IaC_Templates/eks_iam_deployment.yml --capabilities CAPABILITY_NAMED_IAM

echo "Deploy the EKS Infrastructure Stack"
aws cloudformation deploy --stack-name eks-hacking-eks-infrastructure-stack --template-file ./IaC_Templates/eks_infrastructure_deployment.yml
...
TARGET_GROUP_ARN=$(aws cloudformation describe-stacks --stack-name eks-hacking-eks-infrastructure-stack --query "Stacks[0].Outputs[?OutputKey=='EKSHackingALBTargetGroupARN'].OutputValue" --output text)
...
aws cloudformation deploy --stack-name eks-hacking-eks-bh-vpc-peering-stack --template-file ./IaC_Templates/eks_bh_vpc_peering_deployment.yml
...
kubectl apply -f ./kubernetes/vulnerable-web-app-deployment.yml

# Need to create separate file to swap out TARGET_GROUP_ARN with the target group of the created ALB
cp ./kubernetes/vulnerable-web-app-service-template.yml vulnerable-web-app-service.yml
...
kubectl apply -f ./kubernetes/vulnerable-web-app-service.yml
